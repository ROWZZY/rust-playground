...
import { selectClippyCompat, selectFormatCompat, selectMacroExpansionCompat, selectMiriCompat } from './reducers/api';
...
const Output: React.FC = () => {
  ...
  const { meta: { focus }, execute, assembly, llvmIr, mir, hir, wasm, gist } =
    useSelector((state: State) => state.output);
  const format = useSelector(selectFormatCompat);
  const clippy = useSelector(selectClippyCompat);
  const miri = useSelector(selectMiriCompat);
  const macroExpansion = useSelector(selectMacroExpansionCompat);
